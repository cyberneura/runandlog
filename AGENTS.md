...
- **セルごとの Copy ボタン** (CYBERNEURA-DEV-735)。コマンドをクリップボードへ写し、
  ...
  - **クリップボードは `navigator.clipboard` だけを使う。** ボタン 1 つのために
    ...
    (押しても何も起きないボタンにしない)。
  - **書き込みは 1 本のキューに直列化する** (`copyChain`)。並行させると、
    クリップボードに残るのは**ブラウザが最後に書いた方**で、読み手が最後に押した方とは
    限らない。そうなるとチェックが指すセルとクリップボードの中身が食い違う。
    直列化したうえで、追い越された press は `copySeq` を見て黙って降りる。
  - **`forgetCopied` は `copySeq` を必ず進める** (チェックが出ていなくても)。
    応答待ちの press は画面に何も出していないので、番号を進めないと reload の後に
    生き残り、入れ替わったセルへチェックを付ける。
  - **`reload` は `forgetCopied` を 2 回呼ぶ。** invoke の前と、応答が返って描画する
    直前。Copy は reload 中も押せるので、**応答待ちの間に押された press は 1 回目より
    後の番号を持つ**。2 回目が無いと、その press が入れ替わった後のセルを掴む。
